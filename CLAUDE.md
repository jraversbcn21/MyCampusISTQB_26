...
| 3. Glossary expansion | ⏳ Not started | Expand `GLOSSARY` in `js/content.js` with official syllabus keywords; add `source` per term. See spec §"Fase 3". Also sweep `FAQ`/`FLASHCARDS` for stale non-v4.0 terms surfaced by Phase 2 (see below). |
...
- **Not yet fixed:** `FLASHCARDS` array (`js/content.js`, id 28) still lists "false sense of security" as an automation risk — contradicts the Chapter 6 lesson fix above. Flagged by Phase 2's final review as Phase 3 input; the `FAQ`/`GLOSSARY`/`FLASHCARDS` arrays were not in Phase 2's scope (only `CHAPTERS`/`LESSONS` were audited).
